...
// // router.get('/question/:slot', async (req, res) => {
// //     try {
// //         const { language,question_type } = req.query; // Get language from query string

// //         const filter = { slot: req.params.slot };
// //         if (language) {
// //             filter.language_select = language; // Add language filter if provided
// //         }
// //         if (question_type) {
...
// //         const random = Math.floor(Math.random() * count);
// //         const question = await Question.find(filter).limit(1).skip(random);
...
//         // Base filter
//         const filter = { slot: parseInt(slot) , user_id: "" , question_type: question_type};

        
//         if (questionId && is_correct === 'true' && req.session?.user?.id) {
//             await Question.findByIdAndUpdate(questionId, {
//                 $set: { user_id: req.session.user.id }
//             });
//         }
...
//         // Base filter
//         const filter = { slot: parseInt(slot) , question_type: question_type};
...
//         language_select: req.body.language_select
//     });
//     try {
//         const savedQuestion = await question.save();
//         res.status(200).json({ error: null, question: savedQuestion });
//     } catch (err) {
//         res.status(400).json({ error: err });
//     }
// });
...
//         const count = await Question.countDocuments({
//             slot: req.params.slot, question_type: question_type
//         });
